...
// Find locationInfo by uint32
func FindByteByUint(ip uint32) *LocationByteInfo {
	return std.FindByteByUint(ip)
}
...
// Find locationbyteInfo by uint32
func (loc *Locator) FindByteByUint(ip uint32) (info *LocationByteInfo) {
	end := len(loc.indexData1) - 1
	if ip>>24 != 0xff {
		end = loc.index[(ip>>24)+1]
	}
	idx := loc.findIndexOffset(ip, loc.index[ip>>24], end)
	off := loc.indexData2[idx]
	return newLocationByteInfo(loc.textData[off : off+loc.indexData3[idx]])
}
...
func newLocationByteInfo(str []byte) *LocationByteInfo {

	var info *LocationByteInfo

	fields := bytes.Split(str, []byte("\t"))
	switch len(fields) {
	case 4:
		// free version
		info = &LocationByteInfo{
			Country: fields[0],
			Region:  fields[1],
			City:    fields[2],
		}
